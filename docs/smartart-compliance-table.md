...
| Direction `BT` | `graph BT` | 🔧 ❌ None — la mise en page bas-en-haut elle-même reste hors du scope V1 (spec §5.1, `TD`/`LR` seulement) mais depuis le correctif du 2026-09-04, `BT` est **reconnu** par la regex d'en-tête et produit un avertissement explicite (`Direction "BT" is not supported in V1 (only TD/LR); using TD.`) avant de retomber sur `TD`, au lieu de tomber silencieusement dans le message générique "Unsupported line ignored" | 🔧 idem | 🔧 idem |
| Direction `RL` | `graph RL` | 🔧 ❌ None — même limite et même correctif que `BT` ci-dessus (avertissement explicite depuis 2026-09-04, direction toujours hors scope V1) | 🔧 idem | 🔧 idem |
