 ...
 *
 * Usage: node build-custom-chain1-withpres.mjs [base.docx] [custom-chain1-withpres.docx]
 */
...
function fail(msg) {
  console.error(`build-custom-chain1-withpres: ${msg}`);
  process.exit(1);
}
...
function main() {
  ...
  const outDocxArg = process.argv[3] ?? join(HERE, 'custom-chain1-withpres.docx');
  ...
  const dataXml = readFileSync(join(HERE, 'custom-algo', 'data-chain1-withpres.xml'), 'utf8');
  ...
  try {
    ...
    console.log(`build-custom-chain1-withpres: wrote ${outDocx}`);
  } finally {
