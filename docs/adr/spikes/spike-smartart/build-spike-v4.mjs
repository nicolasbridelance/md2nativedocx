 ...
 *
 * Usage: node build-spike-v4.mjs [base.docx] [spike-v4.docx]
 */
...
function fail(msg) {
  console.error(`build-spike-v4: ${msg}`);
  process.exit(1);
}
...
function main() {
  ...
  const outDocxArg = process.argv[3] ?? join(HERE, 'spike-v4.docx');
  ...
  const dataXml = readFileSync(join(HERE, 'hand-data-v4-real-nodrawing.xml'), 'utf8');
  const layoutXml = readFileSync(join(HERE, 'real-diagram1', 'layout1.xml'), 'utf8');
  ...
  try {
    ...
    console.log(`build-spike-v4: wrote ${outDocx}`);
  } finally {
