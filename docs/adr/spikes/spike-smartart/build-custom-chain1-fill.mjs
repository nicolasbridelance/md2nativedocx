 ...
 *
 * Usage: node build-custom-chain1-fill.mjs [base.docx] [custom-chain1-fill.docx]
 */
...
function fail(msg) {
  console.error(`build-custom-chain1-fill: ${msg}`);
  process.exit(1);
}
...
function main() {
  ...
  const outDocxArg = process.argv[3] ?? join(HERE, 'custom-chain1-fill.docx');
  ...
  const dataXml = readFileSync(join(HERE, 'custom-algo', 'data-chain1-withpres-fill.xml'), 'utf8');
  ...
  try {
    ...
    console.log(`build-custom-chain1-fill: wrote ${outDocx}`);
  } finally {
