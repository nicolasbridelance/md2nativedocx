 ...
 *
 * Usage: node build-spike-v3.mjs [base.docx] [spike-v3.docx]
 */
...
function fail(msg) {
  console.error(`build-spike-v3: ${msg}`);
  process.exit(1);
}
...
function main() {
  ...
  const outDocxArg = process.argv[3] ?? join(HERE, 'spike-v3.docx');
  ...
  const dataXml = readFileSync(join(HERE, 'hand-data-v3.xml'), 'utf8');
  ...
  try {
    ...
    console.log(`build-spike-v3: wrote ${outDocx}`);
  } finally {
