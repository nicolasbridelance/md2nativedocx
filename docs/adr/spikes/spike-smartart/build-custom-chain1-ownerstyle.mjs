...
function fail(msg) {
  console.error(`build-custom-chain1-ownerstyle: ${msg}`);
  process.exit(1);
}
...
function main() {
  ...
  const outDocx = resolve(join(HERE, 'custom-chain1-ownerstyle.docx'));
  ...
  const quickStylePath = join(HERE, 'custom-algo', 'quickstyle-chain1.xml');
  ...
  try {
    ...
    console.log(`build-custom-chain1-ownerstyle: wrote ${outDocx}`);
  } finally {
