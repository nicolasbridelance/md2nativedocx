 ...
 *
 * Usage: node build-custom-chain1-mainfill.mjs [base.docx] [custom-chain1-mainfill.docx]
 */
...
function fail(msg) {
  console.error(`build-custom-chain1-mainfill: ${msg}`);
  process.exit(1);
}
...
function main() {
  ...
  const outDocxArg = process.argv[3] ?? join(HERE, 'custom-chain1-mainfill.docx');
  ...
  const dataXml = readFileSync(join(HERE, 'custom-algo', 'data-chain1-mainfill.xml'), 'utf8');
  ...
  try {
    ...
    console.log(`build-custom-chain1-mainfill: wrote ${outDocx}`);
  } finally {
