 ...
 *
 * Usage: node build-spike-v5.mjs [base.docx] [spike-v5.docx]
 */
...
function fail(msg) {
  console.error(`build-spike-v5: ${msg}`);
  process.exit(1);
}
...
function main() {
  ...
  const outDocxArg = process.argv[3] ?? join(HERE, 'spike-v5.docx');
  ...
  const dataXml = readFileSync(join(HERE, 'hand-data-v5.xml'), 'utf8');
  ...
  try {
    ...
    console.log(`build-spike-v5: wrote ${outDocx}`);
  } finally {
