...
function fail(msg) {
  console.error(`build-custom-tree1-styled: ${msg}`);
  process.exit(1);
}
...
function main() {
  ...
  const outDocx = resolve(join(HERE, 'custom-tree1-styled.docx'));
  if (!existsSync(baseDocx)) fail(`base docx not found at ${baseDocx}`);

  const dataXml = readFileSync(join(HERE, 'custom-algo', 'data-tree1-withpres.xml'), 'utf8');
  const layoutXml = readFileSync(join(HERE, 'custom-algo', 'layout-tree1.xml'), 'utf8');
  const colorsPath = join(HERE, 'custom-algo', 'colors-tree1.xml');
  const quickStylePath = join(HERE, 'real-diagram1', 'quickStyle1.xml');
  if (!existsSync(colorsPath) || !existsSync(quickStylePath)) {
    fail(
      'colors-tree1.xml or real-diagram1/quickStyle1.xml not found locally (gitignored, research-only) -- ' +
  ...
  try {
    ...
    console.log(`build-custom-tree1-styled: wrote ${outDocx}`);
  } finally {
