 ...
 *
 * Usage: node build-custom-tree1-withpres.mjs [base.docx] [custom-tree1-withpres.docx]
 */
...
function fail(msg) {
  console.error(`build-custom-tree1-withpres: ${msg}`);
  process.exit(1);
}
...
function main() {
  ...
  const outDocxArg = process.argv[3] ?? join(HERE, 'custom-tree1-withpres.docx');
  ...
  try {
    ...
    console.log(`build-custom-tree1-withpres: wrote ${outDocx}`);
  } finally {
