 ...
 *
 * Usage: node build-custom-chain1.mjs [base.docx] [custom-chain1.docx]
 */
...
function fail(msg) {
  console.error(`build-custom-chain1: ${msg}`);
  process.exit(1);
}
...
function main() {
  ...
  const outDocxArg = process.argv[3] ?? join(HERE, 'custom-chain1.docx');
  ...
  const dataXml = readFileSync(join(HERE, 'custom-algo', 'data-chain1.xml'), 'utf8');
  ...
  try {
    ...
    console.log(`build-custom-chain1: wrote ${outDocx}`);
  } finally {
