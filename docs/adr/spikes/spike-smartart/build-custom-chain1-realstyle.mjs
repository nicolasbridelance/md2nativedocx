...
function fail(msg) {
  console.error(`build-custom-chain1-realstyle: ${msg}`);
  process.exit(1);
}
...
function main() {
  ...
  const outDocx = resolve(join(HERE, 'custom-chain1-realstyle.docx'));
  ...
  const colorsPath = join(HERE, 'real-diagram1', 'colors1.xml');
  const quickStylePath = join(HERE, 'real-diagram1', 'quickStyle1.xml');
  if (!existsSync(colorsPath) || !existsSync(quickStylePath)) {
    fail(
      'real-diagram1/colors1.xml or quickStyle1.xml not found locally (gitignored, research-only) -- ' +
  ...
  try {
    ...
    console.log(`build-custom-chain1-realstyle: wrote ${outDocx}`);
  } finally {
