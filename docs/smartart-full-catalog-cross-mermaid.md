...
| 11 | **Venn / ensembles chevauchants** | Basic Venn, Stacked Venn, Linear Venn, Radial Venn | ❌ nouveau, non construit — **même erreur de jugement que #9**, écarté à tort pour flowchart alors que direct pour `venn-beta` | **`venn-beta`** (correspondance littérale) | Nouveau, et **différent des autres** : le moteur `dgm:` de Word pour un vrai Venn est parmi ses plus complexes en interne ; plus simple et plus fiable de **sortir du moteur SmartArt** pour ce cas et dessiner des ellipses `wps:wsp` positionnées à la main avec transparence (même primitive de forme que le traducteur OOXML flowchart utilise déjà) — visuellement un Venn, techniquement pas un `dgm:dataModel` |
