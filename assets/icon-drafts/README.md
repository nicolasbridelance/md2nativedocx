...
| `icon-v5-measured.svg` (+ `icon-v5-{16,32,128,256,512}.png`) | Vector redraw of `icon-v3`, this time with every internal position/size/color **measured** from the reference crop (pixel-color boundary scans for the diamond vertices, connected-component centroids for the circles/square/diamond/bars). Found and fixed two real mistakes in `icon-v3`: the right half's fill/bar colors were inverted (v3: light fill + purple bars; reference: flat medium-purple fill + white bars), and the left-half graph shapes used the wrong color (v3: light lavender; reference: the same medium purple as the right half's fill). Superseded by `icon-v6` — v5's left-half *topology* (square converging into the diamond) turned out wrong, see below. |
| `icon-v6-corrected.svg` (+ `icon-v6-{16,32,128,256,512}.png`) | Corrects v5's left half per the maintainer's direct geometric read of the reference (2026-09-03): a strict chain **square → circle → diamond → circle**, all four elements the *same* width/height, with the two circles at the ends of an isosceles triangle's vertical base (diamond at the base's midpoint) and the square at the apex, connected to only the nearer circle — not converging into the diamond as v5 had it. Current best candidate. |
